...
# new updates compared with 'efsis_stabPerf.R': 
# 1. times 10 for GeoDE
...
efsis <- function(){
  ...
  for (i in c(1:num.round)){
    ...
    gammas <- 1
    x.train.resample.geode <- x.train.resample * 10
    data.geode <- data.frame(fea.name, x.train.resample.geode)
...
for (num.sel.fea in c(nums.sel.fea)){
  ...
  for (i in c(1:k.folds)){
    ...
    start.time <- proc.time()
    gammas <- 1
    x.train.geode <- x.train * 10
    data.geode <- data.frame(fea.name, x.train.geode)
