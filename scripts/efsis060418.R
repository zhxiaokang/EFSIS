...
# When calculate the stability of a method, instead of using the #sel for that loop, use a higher one (10 times)
# Correction made: using the rankings of features to integrate functions, instead of using the product from previous step (resampling)
# Inherited from 'efsis050418.R', use 10-folds cross validation, and 50 rounds for resampling
# Not using consensus ranking
...
efsis <- function(){
  ...
  for (line in c(1:num.fea)){
    ...
    fea.rank.merge.efsis$final.rank.sam.geode[line] <- (fea.rank.merge.efsis$sam.rank[line])^rank.stab.sam * 
      (fea.rank.merge.efsis$geode.rank[line])^rank.stab.geode
    # integrage Ref, Chs
    rank.stab <- rank(-c(stab.ref, stab.chs))
    rank.stab.ref <- rank.stab[1]
    rank.stab.chs <- rank.stab[2]
    fea.rank.merge.efsis$final.rank.ref.chs[line] <- (fea.rank.merge.efsis$ref.rank[line])^rank.stab.ref * 
      (fea.rank.merge.efsis$chs.rank[line])^rank.stab.chs
    ...
    fea.rank.merge.efsis$final.rank.sam.geode.ref.chs[line] <- (fea.rank.merge.efsis$sam.rank[line])^rank.stab.sam * 
...
#                  auc.efsis.consensus.sam.geode, auc.efsis.consensus.ref.chs, auc.efsis.consensus.sam.geode.ref.chs)
  ...
  write.table(auc.all.folds, paste(path.data, 'num', num.sel.fea, '-', 'auc-stab10-rank-10-50.txt', sep = ''), quote = F, col.names = T, row.names = T)
...
#                 stab.efsis.consensus.sam.geode, stab.efsis.consensus.ref.chs, stab.efsis.consensus.sam.geode.ref.chs)
  ...
  write.table(stab.all, paste(path.data, 'num', num.sel.fea, '-', 'stab-stab10-rank-10-50.txt', sep = ''), quote = F, col.names = F, row.names = F)
}
