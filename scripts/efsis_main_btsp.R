...
for (num.sel.fea in c(nums.sel.fea)){
  ...
  write.table(auc.all.folds, paste(path.data, 'num', num.sel.fea, '-', 'auc-main-btsp.txt', sep = ''), quote = F, col.names = T, row.names = T)
  
  # save the time for this #-sel-fea to file
  write.table(time.all.folds, paste(path.data, 'num', num.sel.fea, '-', 'time-main-btsp.txt', sep = ''), quote = F, col.names = T, row.names = T)
  ...
  write.table(stab.all, paste(path.data, 'num', num.sel.fea, '-', 'stab-main-btsp.txt', sep = ''), quote = F, col.names = F, row.names = F)
}
