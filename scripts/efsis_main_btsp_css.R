...
# Inherited from 'efsis_main.R', use 10-folds cross validation, and 50 rounds for bootstrap resampling, when calculating the stab, use the original # sel fea, no 10 times
...
efsis <- function(){
  ...
  rank.list.infog <- row.names(fea.rank.merge.infog[order(fea.rank.merge.infog[, 'final.rank']), ])
  
  # integrage SAM, GeoDE, Ref, infog
  rank.matrix.sam.geode.ref.infog <- rbind(rank.list.sam, rank.list.geode, rank.list.ref, rank.list.infog)
  invisible(capture.output(rank.list.efsis.sam.geode.ref.infog <- RankAggreg(rank.matrix.sam.geode.ref.infog, num.sel.fea, NULL, method = 'GA', importance = c(rank.stab.sam, rank.stab.geode, rank.stab.ref, rank.stab.infog))))
  sel.fea.css <- rank.list.efsis.sam.geode.ref.infog$top.list
  
  # =============== END of Selecting Features using efsis ===============
  output.list <- list('sel.fea.efsis' = sel.fea.efsis, 'sel.fea.css' = sel.fea.css,
...
for (num.sel.fea in c(nums.sel.fea)){
  ...
  sel.fea.css.folds <- data.frame()
  
  auc.all.folds <- data.frame(row.names = c('sam', 'geode', 'ref', 'infog', 'func', 'EFSIS', 'CSS'))
  ...
  for (i in c(1:k.folds)){
    ...
    # =============== Feature Selection using EFSIS with formular and consensus ranking===============
    ...
    sel.fea.css <- output.list.efsis$sel.fea.css
    end.time <- proc.time()
    ...
    sel.fea.css.folds <- cbind.all(sel.fea.css.folds, sel.fea.css)
    ...
    train.css <- t(x.train[sel.fea.css, ])
    ...
    train.css.svm <- data.frame(cbind(train.css, y.train))
    test.css.svm <- data.frame(test.css)
    ...
    model.css.svm <- svm(formula = factor(y.train)~., data = train.css.svm, probability = TRUE)
    ...
    pred.css.svm <- attr(predict(model.css.svm, test.css.svm, probability = TRUE), 'probabilities')[, 2]
    ...
    auc.css <- auc(y.test, pred.css.svm)
    auc.all <- c(auc.sam, auc.geode, auc.ref, auc.infog, auc.func, auc.efsis, auc.css)
  ...
  write.table(auc.all.folds, paste(path.data, 'num', num.sel.fea, '-', 'auc-main-btsp-css.txt', sep = ''), quote = F, col.names = T, row.names = T)
  
  # save the time for this #-sel-fea to file
  write.table(time.all.folds, paste(path.data, 'num', num.sel.fea, '-', 'time-main-btsp-css.txt', sep = ''), quote = F, col.names = T, row.names = T)
  ...
  stab.css <- stab(sel.fea.css.folds)
  
  stab.all <- c(stab.sam, stab.geode, stab.ref, stab.infog, stab.func, stab.efsis, stab.css)
  # save the stability for this #-sel-fea to file
  write.table(stab.all, paste(path.data, 'num', num.sel.fea, '-', 'stab-main-btsp-css.txt', sep = ''), quote = F, col.names = F, row.names = F)
}
