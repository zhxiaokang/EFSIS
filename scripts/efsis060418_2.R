...
# Inherited from 'efsis050418.R', use 10-folds cross validation, and 50 rounds for resampling, when calculating the stab, use the original # sel fea, no 10 times
...
for (num.sel.fea in c(nums.sel.fea)){
  ...
  write.table(auc.all.folds, paste(path.data, 'num', num.sel.fea, '-', 'auc-rank-10-50.txt', sep = ''), quote = F, col.names = T, row.names = T)
  ...
  write.table(stab.all, paste(path.data, 'num', num.sel.fea, '-', 'stab-rank-10-50.txt', sep = ''), quote = F, col.names = F, row.names = F)
}
