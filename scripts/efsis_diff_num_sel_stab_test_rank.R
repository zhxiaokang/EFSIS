...
# Inherited from 'efsis_diff_num_sel_stab_test.R', but using rankings of stab, instead of orignal stab value (w)
remove(list = ls())
...
efsis <- function(){
  ...
  for (line in c(1:num.fea)){
    ...
    fea.rank.merge.efsis$final.rank.sam.geode[line] <- (fea.rank.merge.sam$final.rank[line])^rank.stab.sam * 
      (fea.rank.merge.geode$final.rank[line])^rank.stab.geode
    # integrage Ref, Chs
    rank.stab <- rank(-c(stab.ref, stab.chs))
    rank.stab.ref <- rank.stab[1]
    rank.stab.chs <- rank.stab[2]
    fea.rank.merge.efsis$final.rank.ref.chs[line] <- (fea.rank.merge.ref$final.rank[line])^rank.stab.ref * 
      (fea.rank.merge.chs$final.rank[line])^rank.stab.chs
    ...
    fea.rank.merge.efsis$final.rank.sam.geode.ref.chs[line] <- (fea.rank.merge.sam$final.rank[line])^rank.stab.sam * 
      (fea.rank.merge.geode$final.rank[line])^rank.stab.geode * (fea.rank.merge.ref$final.rank[line])^rank.stab.ref * 
      (fea.rank.merge.chs$final.rank[line])^rank.stab.chs
  }
